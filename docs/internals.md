...
Discord-originated reply dispatch의 prompt refinement 규칙은 `skills/tm-send/SKILL.md`에 정리되어 있습니다.
...
`tm-send --discord-approval`은 bridge-managed approval question을 만들 뿐입니다. Discord 버튼/선택 UI는 Hermes `clarify`/AskUserQuestion 렌더링이 별도로 필요합니다.
...
- `tm-new`가 `gjc` 기반 managed tmux session을 만들고 `@gjc-*` ownership tag를 검증하는지.
- `tm-send`/`tm-kill`이 raw tmux fallback 없이 bridge API/audit path를 사용하는지.
...
bash -n bin/tm-new bin/tm-send bin/tm-kill scripts/*.sh bin/install.sh
npm test
