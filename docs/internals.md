...
`hermes-omx-notify`는 OMX lifecycle record와 로컬 Codex JSONL/log를 canonical runtime evidence로 봅니다. tmux list, delivery cursor, event index, channel map은 재생성 가능한 운영 상태입니다. 서로 다른 session을 억지로 하나로 합쳐 두 번째 truth를 만들지 않는 것이 우선입니다.
...
Discord-originated reply dispatch는 `skills/omx-send/SKILL.md`가 prompt refinement SSoT입니다.
...
`omx-send --discord-approval`은 bridge-owned approval question을 만들 뿐입니다. Discord 버튼/선택 UI는 Hermes `clarify`/AskUserQuestion 렌더링이 별도로 필요합니다.
