# Quick Start — Hermes Codex Notify

영어 기본 문서는 [`docs/quickstart.md`](quickstart.md)입니다.

이 문서는 이미 Hermes Gateway와 Discord bot이 있는 self-host 환경에서 `hermes-codex-notify`를 빠르게 붙이는 절차입니다. Hermes/에이전트용 단일 runbook은 repository root의 `INSTALL.md`입니다.
...
codex / Codex / tmux
  ...
  -> Hermes subscription `codex-notify` + hermes-codex-notify skill
...
openssl rand -hex 32 > ~/.config/hermes-codex-notify/hermes-webhook.secret
chmod 600 ~/.config/hermes-codex-notify/hermes-webhook.secret
```

채널 매핑 예시:

```bash
cat > ~/.config/hermes-codex-notify/project-channels.json <<'JSON'
{
...
JSON
chmod 600 ~/.config/hermes-codex-notify/project-channels.json
```
...
```text
hermes-codex-notify로 bridge health와 sessions를 확인해줘.
```
