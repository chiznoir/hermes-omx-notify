# Hermes Gateway ↔ hermes-tmux-bridge 연결 가이드

이 문서는 Hermes Gateway와 `hermes-tmux-bridge`를 연결하는 운영 가이드입니다. 빠른 설치는 [`docs/quickstart-ko.md`](quickstart-ko.md)를 먼저 보세요.
...
Discord user
  ...
  -> hermes-tmux-bridge skill
  -> hermes-tmux-bridge HTTP API
...
```text
hermes-tmux-bridge event router
  -> Hermes webhook sink
  -> POST /webhooks/tmux-bridge
...
생략하는 기본값: `HOST=127.0.0.1`, `PORT=3037`, `BRIDGE_PUBLIC_URL=http://127.0.0.1:3037`, `BRIDGE_HERMES_PROJECT_CHANNEL_MAP=~/.config/hermes-tmux-bridge/project-channels.json`, `BRIDGE_HERMES_NOTIFICATION_MODE=direct`.
...
scripts/install-systemd-service.sh \
  ...
  --sink-secret-file ~/.config/hermes-tmux-bridge/hermes-webhook.secret \
  --channel <fallback-channel-id> \
  --map ~/.config/hermes-tmux-bridge/project-channels.json
```
...
Hermes는 `hermes-tmux-bridge` skill을 통해 다음을 수행합니다.
...
- `tm-kill`로 검증된 managed tmux 세션 종료
...
Helper CLI는 이 repository의 `bin/` 기준입니다. `scripts/install-hermes-stack.sh`가 기본으로 `tm-new`/`tm-send`/`tm-kill`을 `PATH`에 설치하며, CLI만 설치하려면 `scripts/install-omx-cli.sh --force`를 사용합니다. 이 CLI installer는 Codex global hook을 수정하지 않습니다. `tm-new` 기본 형태는 `tm-new [a] [PROJECT_DIR] [--name SESSION] [--attach] [--direct] [--json] [--runs PATH] [--no-check] [-- OMX_ARGS...]`이며 OMX session을 시작합니다. GJC는 명시적으로 `tm-new [a] --gjc [PROJECT_DIR] [--name SESSION] [--worktree PATH] [--json] [--no-check] [-- GJC_ARGS...]`를 사용합니다. `a`는 attach shorthand이고, 디렉터리 `a`는 `./a`로 지정합니다. Hermes가 별도 `gjc-new` helper, legacy alias, raw unmanaged target dispatch를 기본 경로로 만들지 않도록 유지합니다.
