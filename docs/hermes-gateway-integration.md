# Hermes Gateway ↔ hermes-omx-bridge 연결 가이드

이 문서는 Hermes Gateway와 `hermes-omx-bridge`를 연결하는 운영 가이드입니다. 빠른 설치는 [`docs/quickstart-ko.md`](quickstart-ko.md)를 먼저 보세요.
...
```text
hermes-omx-bridge event router
  -> Hermes webhook sink
  -> POST /webhooks/omx-bridge
...
생략하는 기본값: `HOST=127.0.0.1`, `PORT=3037`, `BRIDGE_PUBLIC_URL=http://127.0.0.1:3037`, `BRIDGE_HERMES_PROJECT_CHANNEL_MAP=~/.config/hermes-omx-bridge/project-channels.json`, `BRIDGE_HERMES_NOTIFICATION_MODE=direct`.
...
scripts/install-systemd-service.sh \
  ...
  --sink-secret-file ~/.config/hermes-omx-bridge/hermes-webhook.secret \
  --channel <fallback-channel-id> \
  --map ~/.config/hermes-omx-bridge/project-channels.json
```
...
Hermes는 `hermes-omx-bridge` skill을 통해 다음을 수행합니다.
