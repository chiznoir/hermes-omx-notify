# Operations — hermes-omx-bridge

## 서비스

`hermes-omx-bridge`는 기본 설치에서 **systemd user service**로 동작합니다.
system-wide 목록인 `systemctl list-units`에는 안 보일 수 있으므로 `--user`를 붙입니다.

```bash
systemctl --user list-units --type=service --all | grep -E 'hermes-omx-bridge|hermes-gateway'
systemctl --user status hermes-omx-bridge.service
systemctl --user status hermes-gateway.service
systemctl --user cat hermes-omx-bridge.service
journalctl --user -u hermes-omx-bridge.service -f
...
```text
systemd --user hermes-omx-bridge.service
...
```bash
pid="$(systemctl --user show -p MainPID --value hermes-omx-bridge.service)"
ps -fp "$pid" --forest
...
Hermes skill 경계는 역할별로 분리합니다. `hermes-omx-bridge`는 bridge read/status/notification rendering 전용이고, 생성·전달·종료 mutation은 전용 helper skill이 소유합니다. 알림 reply에서 “이 세션/이거/방금 알림”은 항상 해당 알림의 `bridge_session_id`를 뜻하며 project 최신 세션으로 추정하지 않습니다. “이건 뭐냐”, “무슨 뜻이야?”, “궁금한데”, “왜 그래?” 같은 단순 설명 요청은 dispatch/stop/start 단어가 없을 때만 `hermes-omx-bridge`가 bridge read endpoint로 확인해 설명합니다.

전용 mutation skill mapping은 다음과 같습니다.

- `omx-new`: “새 세션”, “세션 열어”, “시작해”, create/launch/start/watch a new OMX session. Bridge webhook `SessionStart` 알림 본문은 새 세션 트리거가 아닙니다.
- `omx-send`: “전달”, “보내”, “넘겨”, “세션에 전달”, “follow-up”, “이거 반영”, “수정”, “고쳐”, “계속”, AskPermission의 `/approve`/`/deny` 같은 승인/거절. Discord-originated Hermes reply dispatch는 정제 후 `omx-send --discord-approval`로 bridge pending approval을 만들고, Hermes가 이어서 native `clarify`/AskUserQuestion으로 operator 전송/거절/추가수정 카드를 실제 Discord에 렌더링해야 합니다. `omx-send --discord-approval`의 JSON `component_actions`만으로는 Gateway 버튼이 자동 생성되지 않습니다. `/new`/`/resume`이 기존 OMX/Codex tmux pane에 전달되는 프롬프트 안에 있으면 Codex slash command이므로 `omx-send`가 원문 의미를 보존해 전달합니다. prompt refinement SSoT도 `skills/omx-send/SKILL.md`입니다.
- `omx-kill`: “종료”, “이 세션 종료해”, “kill”, “킬”, “죽여”, stop/close session. reply 대상 `bridge_session_id`나 정확한 `tmux` metadata를 우선합니다.
- `hermes-omx-bridge`: “원문 알려줘”, “마지막 답변 원문”, “raw”, “full text”는 `/sessions/:id/idle/latest`의 `fullText`; “세션 열려있어?”, “세션 살아있어?”, “현재 세션 확인”은 `/sessions` 또는 `/sessions/:id/state`; “최근 로그/이벤트”는 `/sessions/:id/events`; “내가 보낸 이력/명령 이력”은 `/sessions/:id/interactions`.
...
- `skills/hermes-omx-bridge/SKILL.md`: 브리지 운영/조회/상태확인/알림 렌더링 skill입니다. Hermes가 `omx-bridge` subscription에서 함께 로드하며 `scripts/install-hermes-skill.sh`가 `~/.hermes/skills/autonomous-ai-agents/hermes-omx-bridge/`로 복사합니다.
- `skills/omx-new/SKILL.md`, `skills/omx-send/SKILL.md`, `skills/omx-kill/SKILL.md`: 세션 생성, 프롬프트 전달, 세션 종료를 각각 소유하는 전용 helper skills입니다. `omx-send`에는 prompt refinement, 과정제/의미왜곡 금지선, temp-file/write_file 금지선, Discord-originated `--discord-approval` + Hermes `clarify` card 렌더링 규칙이 들어 있습니다. `scripts/install-hermes-stack.sh`의 webhook subscription은 `hermes-omx-bridge,omx-new,omx-send,omx-kill`을 함께 로드합니다.
- `src/hermes-webhook-sink.js`: 알림 대상 이벤트와 payload를 만드는 코드 경계입니다. `shouldPollSession`/`shouldForwardToHermes`가 OMX-owned lifecycle 및 native-only 오염 필터를 적용합니다.
- `src/server.js`: `omx-send`/`POST /sessions/:id/commands` dispatch 응답을 만들고, 입력 원문은 `bridge-interactions` 기반 `CommandSubmitted` / `User Command` 이벤트로 노출되게 합니다.
...
`omx-new`/`omx-send`/`omx-kill` helper CLI의 SSoT는 이제 이 repository의 `bin/` 디렉터리입니다. `scripts/install-omx-cli.sh`는 세 CLI만 `PATH`에 설치하며 Codex global hook은 수정하지 않습니다. `scripts/install-hermes-stack.sh`는 기본으로 이 CLI installer를 먼저 실행한 뒤 Hermes skill과 bridge service를 설치합니다.

- CLI만 설치/갱신: `scripts/install-omx-cli.sh --force`
...
현재 `omx-new`는 `omx-new [PROJECT_DIR] [--name SESSION] [--attach] [--direct] [--json] [--runs PATH] [--no-check] [-- OMX_ARGS...]`입니다. Hermes가 새 visible session을 시작할 때는 raw `omx --madmax --high` 대신 `omx-new`를 사용합니다. `omx-new`가 기본 모드에서 native tmux session과 `omx --madmax --high`를 구성하고, 명시적 `--direct`/`-d`에서는 tmux session 없이 현재 터미널에서 `omx --direct --yolo`를 실행하므로, Hermes 규칙에서 `--tmux`, `--direct`, `--disable codex_hooks`를 기본값처럼 추가하면 안 됩니다. `--direct`/`-d`는 `--attach`/`-a`, `--name`, `--json`과 함께 사용할 수 없습니다.
...
- `omx-*` 세션의 `SessionStart` 또는 `SessionEnd` 알림은 옵니다.
...
- 해당 세션의 `sessionLogPath`가 `null`이거나, `.omx/state/*hud-state.json`의 `turn_count`가 0으로 남습니다.
...
rg -n '<prompt 일부>|<thread id>|<project path>' ~/.codex/sessions ~/.omx/logs ~/.omx-runs 2>/dev/null
```

원인:

Codex App/remote surface가 연결된 세션에서는 실제 tmux process cwd가 프로젝트여도 Codex rollout JSONL의 `session_meta.cwd`와 native hook payload `cwd`가 `$HOME` 같은 다른 root로 기록될 수 있습니다. 이때 native hook의 `turns-*.jsonl`도 프로젝트 `.omx/logs`가 아니라 `$HOME/.omx/logs`에 생깁니다. bridge는 visible OMX session의 canonical root/run root에 붙은 Codex log만 같은 세션의 `CommandSubmitted`/`FinalAnswer`로 취급하므로, cwd가 갈라진 native thread를 조용히 `omx-*` 세션에 붙이지 않습니다. 잘못 붙이면 다른 열린 Codex thread의 답변이 해당 프로젝트 thread에 `Session Idle`처럼 전송됩니다.

복구/운영:

- `$HOME/.omx/logs/turns-*.jsonl`에 답변이 있으면 Codex hook 자체는 돈 것입니다. 문제는 delivery가 아니라 session reconciliation입니다.
- native thread를 별도 세션으로라도 보고 싶으면 실제 hook cwd root를 `BRIDGE_PROJECT_ROOTS`에 추가할 수 있습니다. 단, 이 방법은 그 native thread를 원래 `omx-*` project session으로 re-label하지 않습니다.
- 프로젝트 thread에 `User Command`/`Session Idle`을 보내려면 Codex가 남기는 `session_meta.cwd`와 native hook `cwd`가 프로젝트 root 또는 OMX run root로 일치해야 합니다. 그렇지 않은 상태에서 bridge가 final을 만들어 보내는 것은 금지합니다.
...
```text
Hermes webhook failed: 404 {"error":"Unknown route: omx-bridge"}
```
...
해결: `omx-bridge` subscription을 다시 생성하고 Hermes Gateway를 재시작합니다.
...
```text
# Session Idle
**session:** `omx-bridge-102122`
**tmux:** `omx-bridge-102122` | **project:** `hermes-omx-bridge`
```
...
bridge는 Codex JSONL/OMX logs/tmux list에서 다시 index를 만듭니다. 살아있는 tmux session과 Codex log가 있으면 재연결됩니다.

## 안전성

- 로컬-only는 `HOST=127.0.0.1` 권장.
- LAN/Docker/reverse proxy/public bind면 `OMX_BRIDGE_TOKEN` 필수.
- webhook secret과 Discord webhook URL은 repo에 커밋하지 않습니다.
- `.env`, `~/.config/hermes-omx-bridge/*.env`, secret files는 로컬 운영 파일입니다.
