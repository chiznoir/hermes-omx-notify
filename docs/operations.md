...
`omx-new`, `omx-send`, `omx-kill`의 source of truth는 repository's `bin/`입니다. 설치/갱신은 아래 명령만 사용합니다.
...
- `skills/omx-new/SKILL.md`: 새 OMX-owned lifecycle session 생성 규칙입니다.
- `skills/omx-send/SKILL.md`: 기존 session 전달 규칙입니다. prompt refinement SSoT이므로 의미 보존, routing metadata 제거, Discord approval flow를 여기에서만 바꿉니다.
