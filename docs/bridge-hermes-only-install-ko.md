...
이 문서는 새 PC나 새 Hermes Agent에 **hermes-omx-bridge와 Hermes/Discord 알림 연동만** 설치할 때 사용합니다. AgentMemory, CodeGraph, Cognee, Codex MCP memory/search 도구는 설치하지 않습니다.

## 목표

설치 후 아래만 동작하면 완료입니다.

1. `hermes-omx-bridge` server가 `127.0.0.1:3037`에서 systemd user service로 실행된다.
2. Hermes에 `hermes-omx-bridge`, `omx-new`, `omx-send`, `omx-kill` skill이 설치된다.
3. Hermes Gateway webhook subscription `omx-bridge`가 `AskPermission,FinalAnswer` 이벤트를 받는다.
4. Discord project channel 또는 session thread로 알림이 전달된다.
5. `omx-new`, `omx-send`, `omx-kill` helper CLI가 `PATH`에 있다.

필요한 helper CLI는 `scripts/install-hermes-stack.sh`가 `omx-new`, `omx-send`, `omx-kill`만 설치합니다. CLI만 설치할 때는 `bin/install.sh --force` 또는 `scripts/install-omx-cli.sh --force`를 사용합니다.
...
- project별 Discord channel mapping: 예) `hermes-omx-bridge=345678901234567890`
...
```bash
omx-send --session <bridge-session-id-or-tmux-id> '짧게 현재 cwd와 작업 가능 여부만 답해줘.'
```
...
```bash
systemctl --user status hermes-omx-bridge.service --no-pager
journalctl --user -u hermes-omx-bridge.service -n 100 --no-pager
...
- `~/.config/hermes-omx-bridge/hermes-omx-bridge.env`에는 켠 기능, secret/token 경로 또는 값, non-default override만 둡니다.
