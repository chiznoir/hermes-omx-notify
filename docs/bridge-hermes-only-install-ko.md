...
이 문서는 새 PC나 새 Hermes Agent에 **hermes-omx-notify와 Hermes/Discord 알림 연동만** 설치할 때 사용합니다. AgentMemory, CodeGraph, Cognee, Codex MCP memory/search 도구는 설치하지 않습니다.
...
2. Hermes에 `hermes-omx-notify`, `omx-new`, `omx-send`, `omx-kill` skill이 설치된다.
...
- project별 Discord channel mapping: 예) `hermes-omx-notify=345678901234567890`
...
```bash
systemctl --user status hermes-omx-notify.service --no-pager
journalctl --user -u hermes-omx-notify.service -n 100 --no-pager
...
- `~/.config/hermes-omx-notify/hermes-omx-notify.env`에는 켠 기능, secret/token 경로 또는 값, non-default override만 둡니다.
