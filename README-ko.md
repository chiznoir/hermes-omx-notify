...
**Hermes, OMX, Codex, tmux 기반 에이전트 세션을 위한 localhost 우선 bridge core.**
...
`hermes-omx-bridge`는 **bridge core**입니다. Hermes가 로컬 OMX/Codex 세션을 다룰 수 있도록 작은 HTTP service와 helper CLI를 제공합니다. 로컬 log와 tmux에서 세션을 찾고, Codex 답변 원문을 읽고, visible tmux pane에 후속 명령을 전달하며, 선택된 이벤트를 Hermes Gateway를 통해 Discord로 보냅니다.

```text
Hermes / Discord
  -> 127.0.0.1에서 실행되는 hermes-omx-bridge
...
Hermes가 개발자와 같은 로컬 근거를 볼 수 있으면 작업을 더 안정적으로 오케스트레이션할 수 있습니다. 이 bridge는 OMX lifecycle log, Codex JSONL transcript, tmux pane, command/audit history를 localhost 안에서 읽고 Hermes에 필요한 좁은 API만 제공합니다.

다음 상황에서 사용합니다.

- Hermes에서 로컬 OMX/Codex 세션을 시작, 조향, 종료하고 싶을 때
...
- **내장 helper CLI** — `bin/`에 bridge lifecycle 도구(`omx-new`, `omx-send`, `omx-kill`)를 포함합니다.
...
| Helper CLI only | `bin/install.sh --force` | `omx-new`, `omx-send`, `omx-kill`만 `PATH`에 올리면 될 때. 내부적으로 `scripts/install-omx-cli.sh`를 감쌉니다. |
...
- Docker, LAN, reverse proxy, public internet에서 접근 가능해지면 `OMX_BRIDGE_TOKEN`을 설정합니다.
...
`OMX_BRIDGE_TOKEN`이 설정되어 있으면 `GET /health`를 제외한 모든 endpoint에 아래 header가 필요합니다.
...
- `tmux` — visible tmux target에 text를 보냅니다. OMX/team session에 적합합니다.
...
bridge는 로컬 OMX/Codex log를 canonical runtime evidence로 취급합니다. derived index, cache, delivery cursor는 다시 만들 수 있는 상태이며 두 번째 source of truth가 되면 안 됩니다.
...
- [Helper CLI docs](bin/README.md) — `omx-new`/`omx-send`/`omx-kill` 운영 기준.

`omx-new`, `omx-send`, `omx-kill` helper 계약의 SSoT는 `bin/`이고 bridge lifecycle 설치는 `bin/install.sh`, `scripts/install-omx-cli.sh`, 또는 `scripts/install-hermes-stack.sh`가 담당합니다. 스크립트 내용을 문서에 중복 복사하지 않습니다.
