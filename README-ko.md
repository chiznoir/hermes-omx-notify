...
`hermes-codex-notify`는 **notification bridge**입니다. Hermes가 로컬 Codex 세션 이벤트를 받고 필요한 후속 동작을 할 수 있도록 작은 HTTP service, Codex hook layer, helper CLI를 제공합니다. Hook layer는 기본 설치되며 lifecycle/final-answer 이벤트를 전체 transcript polling이 아니라 Codex runtime event로 기록합니다. Codex JSONL parsing은 진단용 fallback으로 남깁니다.
...
- **Codex hook layer** — bridge-owned `SessionStart`, `UserPromptSubmit`, `Stop`, `notify` handler를 기본 설치합니다. 다른 harness가 Codex hook을 소유할 때만 `--no-hooks`를 사용하세요.
- **세션 발견** — bridge-owned hook/lifecycle record, Codex JSONL fallback session, tmux pane을 하나의 bridge session view로 병합합니다.
- **전체 출력 조회** — hook이 기록한 final-answer 이벤트를 우선 사용하고, Codex session log는 fallback으로 유지합니다.
...
Stack/service installer는 Codex hook layer를 기본 설치합니다. 실제 실행할 때만 대상 `CODEX_HOME`을 수정하며, 테스트 설치는 `--codex-home <path>`, hook 등록 생략은 `--no-hooks`를 사용하세요. 서비스는 **systemd user service**로 설치됩니다. system-wide `systemctl ...`이 아니라 `systemctl --user ...`를 사용하세요.
