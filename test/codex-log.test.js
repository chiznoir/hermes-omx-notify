...
test('readCodexLog extracts OMX runtime owner from session_meta base instructions', async () => {
  const root = await mkdtemp(join(tmpdir(), 'omx-codex-log-runtime-'));
    ...
    payload: {
      ...
      base_instructions: {
        text: [
          '<!-- OMX:RUNTIME:START -->',
          '<session_context>',
          '**Session:** omx-1778721494742-58n1q1 | 2026-05-14T01:18:14.915Z',
  ...
  assert.equal(log.runtimeOmxSessionId, 'omx-1778721494742-58n1q1');
  assert.equal(log.runtimeTmuxId, 'omx-docs-101814');
  assert.equal(log.runtimeSessionContextSource, 'omx-runtime-block');
});

test('readCodexLog extracts subagent metadata from session_meta', async () => {
  const root = await mkdtemp(join(tmpdir(), 'omx-codex-log-subagent-'));
...
test('readCodexLog marks omx explore codex exec logs as auxiliary', async () => {
  const root = await mkdtemp(join(tmpdir(), 'omx-codex-log-explore-'));
  ...
  assert.equal(log.isOmxExploreHarness, true);
  assert.equal(isAuxiliaryCodexLog(log), true);
});

test('readCodexLog does not treat normal OMX instructions mentioning omx explore as auxiliary', async () => {
  const root = await mkdtemp(join(tmpdir(), 'omx-codex-log-normal-explore-mention-'));
  ...
  await writeFile(logPath, JSON.stringify({
    ...
    payload: {
      ...
      timestamp: '2026-05-19T08:00:00.000Z',
      cwd: '/home/user/work/omx-bridge',
      base_instructions: { text: 'Explore Command Preference: Use `omx explore --prompt ...` for simple lookups.' },
    },
  }), 'utf8');

  const log = await readCodexLog(logPath);
  assert.equal(log.isOmxExploreHarness, false);
  assert.equal(isAuxiliaryCodexLog(log), false);
});

test('readCodexLog exposes pending permission calls', async () => {
  const root = await mkdtemp(join(tmpdir(), 'omx-codex-log-pending-permission-'));
...
test('readCodexLog suppresses resolved permission calls', async () => {
  const root = await mkdtemp(join(tmpdir(), 'omx-codex-log-resolved-permission-'));
