...
Install a same-host `hermes-codex-notify` stack that provides:

1. `codex-new`, `codex-send`, and `codex-kill` helper CLIs on `PATH`.
2. The `hermes-codex-notify` Hermes skill.
...
scripts/install-systemd-service.sh \
  ...
  --secret-file ~/.config/hermes-codex-notify/hermes-webhook.secret \
  --channel <fallback-discord-channel-id> \
...
hermes webhook list | grep codex-notify
journalctl --user -u hermes-codex-notify.service --no-pager -n 100 | grep 'bridge Hermes webhook sink enabled'
```
