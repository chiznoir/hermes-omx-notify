...
usage() {
  cat <<'USAGE'
Install the recommended self-hosted Hermes tmux bridge stack.
...
What it does:
  1. installs tm-new/tm-send/tm-kill helper CLIs from this repository
...
Options:
  ...
  --skip-cli                     Do not install tm-new/tm-send/tm-kill
  ...
  --no-webhook                   Disable webhook sink and remove tmux-bridge subscription (default)
...
config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/hermes-tmux-bridge"
...
bridge_port="3037"
gateway_url="${BRIDGE_HERMES_WEBHOOK_URL:-http://127.0.0.1:8644/webhooks/tmux-bridge}"
...
prompt_missing_inputs() {
  ...
  echo "Hermes tmux bridge interactive setup"
...
라우팅: reply의 “이 세션/이거/방금 알림”은 payload `bridge_session_id`. 최우선 dispatch: Discord reply에서 `전달`/`보내`/`넘겨`/`세션에 넣어`면 설명/조회하지 말고 정제 후 `tm-send --session <bridge_session_id> --discord-approval`; 반영/수정/재시도/진행/계속/위 내용/답장/고쳐는 “전달해” 없어도 정제 후 `tm-send --discord-approval`. 이 gate는 bridge pending approval만 만들며 Discord 버튼을 자동 렌더링하지 않는다. `delivery.status=approval-pending`이면 반드시 Hermes native `clarify`/AskUserQuestion으로 정제 prompt와 선택지 `전송`, `거절`, `추가수정`을 표시한 뒤, 선택 결과를 `tm-send --session <bridge_session_id> --answer-approval send|reject --question-id <questionId>`로 제출하고, 추가수정은 편집 문구를 받은 뒤 `--answer-approval modify --question-id <questionId> --edit <text>`로 기존 pending을 닫고 재정제한다. `clarify` 성공 전에는 “승인 요청 올려뒀다/버튼에서 전송 누르면 된다”고 말하지 말고, 실패 시 approval UI 실패를 명시한다. 추가수정은 재정제 후 새 approval을 만든다. Discord-origin이 아닌 명시적 local/manual helper dispatch만 plain `tm-send --session <bridge_session_id>` 가능. “이건 뭐냐/궁금한데/왜 그래?”는 dispatch 단어가 없을 때만 읽고 설명. 종료/kill은 `tm-kill` skill; 새 세션은 `tm-new` skill; dispatch는 `tm-send` skill; `SessionStart` 트리거 아님. 새 세션 default는 OMX: `tm-new [PROJECT_DIR]`. GJC는 명시 요청 때만 `tm-new --gjc [PROJECT_DIR]`, worktree는 `tm-new --gjc --worktree PATH`. 별도 `gjc-new` helper나 legacy alias를 만들지 않는다. 직접 `--tmux` target은 GJC managed 경로에만 제한하고, OMX는 bridge `--session`으로 보낸다. `/new`/`/resume`은 Codex slash command라 `tm-send`로 그대로 보내되 Discord reply라면 `--discord-approval` gate를 먼저 거친다. 원문/raw/full text는 `/sessions/:id/idle/latest` `fullText`; 세션 확인은 `/sessions` 또는 `/sessions/:id/state`; 최근 로그 `/sessions/:id/events`; 명령 이력은 `/sessions/:id/interactions`. AskPermission: `/approve`/`/deny`를 `tm-send --mode tmux`.

대상/금지: 새 session lifecycle은 OMX 기본 + 명시적 GJC opt-in만. `channel_mapping_status=session-thread`: `channel_id`/`discord_delivery_target_id`/`chunk_delivery_channel_id`는 이미 Discord thread다. 긴 direct FinalAnswer는 bot token과 thread target이 있으면 브리지가 Discord에 직접 순차 전송한다. Hermes에 chunked payload가 도착한 경우에는 브리지가 1800자 안전 한도로 미리 나눈 각 payload를 순서대로 같은 thread에 보내고 재탐색/fallback 금지. 알림=응답; 확인 금지. `알림 렌더링 완료`/`요약을 보냈고` 금지.
...
전용 skill 경계: `hermes-tmux-bridge`는 bridge read/status/notification rendering만 맡는다. 세션 생성은 `tm-new`, 전달/승인/거절은 `tm-send`, 종료는 `tm-kill` skill을 따른다. `tm-send` prompt refinement 규칙은 `skills/tm-send/SKILL.md`에 있으며, temp file/write_file을 쓰더라도 원문 Discord reply가 아니라 정제된 prompt만 기록해야 한다. Discord-originated Hermes dispatch는 정제된 prompt를 바로 보내지 말고 `tm-send --discord-approval`로 bridge-managed `tmux-send-approval` question을 만든 다음 `clarify`/AskUserQuestion으로 실제 Discord 승인 카드를 렌더링해야 한다.
...
install_subscription() {
  ...
  if [[ "$dry_run" == "1" ]]; then
    echo "DRY-RUN: install/update Hermes webhook subscription tmux-bridge in $hermes_home"
    return
  fi
  ...
  if command -v hermes >/dev/null 2>&1; then
    if subscribe_output="$(HERMES_HOME="$hermes_home" hermes webhook subscribe tmux-bridge \
      --events AskPermission,FinalAnswer \
      --skills hermes-tmux-bridge,tm-new,tm-send,tm-kill \
      --deliver discord \
...
data['tmux-bridge'] = {
  description: 'Bridge events from hermes-tmux-bridge for project-channel summarization',
  events: ['AskPermission', 'FinalAnswer'],
...
remove_subscription() {
  if [[ "$dry_run" == "1" ]]; then
    echo "DRY-RUN: remove Hermes webhook subscription tmux-bridge from $hermes_home if present"
    return
  fi
...
if (!data || typeof data !== 'object' || !Object.prototype.hasOwnProperty.call(data, 'tmux-bridge')) process.exit(0);
delete data['tmux-bridge'];
fs.writeFileSync(file, `${JSON.stringify(data, null, 2)}\n`, { mode: 0o600 });
fs.chmodSync(file, 0o600);
NODE
}
...
Hermes tmux bridge stack install complete.
...
Check:
  ...
  systemctl ${scope_systemctl_arg} status hermes-tmux-bridge.service
