...
usage() {
  cat <<'USAGE'
Apply the checked-out hermes-codex-notify repo to the local runtime service.
...
Examples:
  ...
  scripts/apply-runtime.sh --system --name hermes-codex-notify --port 3037
USAGE
}

scope="user"
name="${BRIDGE_SERVICE_NAME:-hermes-codex-notify}"
host="${HOST:-127.0.0.1}"
port="${PORT:-3037}"
health_url=""
