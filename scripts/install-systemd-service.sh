...
usage() {
  cat <<'USAGE'
Install hermes-tmux-bridge as a systemd service.
...
Options:
  ...
  --sink-url URL
                           Hermes webhook endpoint (default: http://127.0.0.1:8644/webhooks/tmux-bridge)
  --secret SECRET
...
Examples:
  scripts/install-systemd-service.sh --token-file ~/.config/hermes/tmux-bridge.token
  scripts/install-systemd-service.sh --system --state-root /var/lib/hermes-tmux-bridge --token 'change-me'
USAGE
}
...
hermes_webhook_url="${BRIDGE_HERMES_WEBHOOK_URL:-http://127.0.0.1:8644/webhooks/tmux-bridge}"
...
if [[ "$scope" == "user" ]]; then
  ...
  default_env_dir="${XDG_CONFIG_HOME:-$HOME/.config}/hermes-tmux-bridge"
  default_state_root="${XDG_STATE_HOME:-$HOME/.local/state}/hermes-tmux-bridge"
  systemctl_cmd=(systemctl --user)
  service_path="$service_dir/$name.service"
else
  service_dir="/etc/systemd/system"
  default_env_dir="/etc/hermes-tmux-bridge"
  default_state_root="/var/lib/hermes-tmux-bridge"
...
Description=Hermes tmux bridge
...
user_default_channel_map="${XDG_CONFIG_HOME:-$HOME/.config}/hermes-tmux-bridge/project-channels.json"
