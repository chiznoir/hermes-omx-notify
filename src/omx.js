...
async function readMadmaxRunMetadata(root) {
  ...
  try {
    ...
    console.error(`[tmux-bridge] failed to read madmax run metadata ${madmaxRunMetadataPath(root)}: ${error?.message || error}`);
    return null;
  }
}
...
async function readTerminalPendingProjectRootRecords(projectRoot, options = {}) {
  ...
  try {
    ...
    if (options.warnOnTerminalPendingRootDiscoveryError !== false) {
      console.error(`[tmux-bridge] failed to read terminal pending roots from event index ${path}: ${error?.message || error}`);
    }
    return [];
  } finally {
    try {
...
async function readDiscoveredProjectRootRecords(projectRoot, options = {}) {
  ...
  try {
    ...
    if (options.warnOnDiscoveredProjectRootRegistryError !== false) {
      console.error(`[tmux-bridge] failed to read discovered project root registry ${path}: ${error?.message || error}`);
    }
    return [];
  }
}
